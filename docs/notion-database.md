...
本项目使用一个 Notion 数据库作为博客内容源。作者只需填写文章名称和页面正文；发布状态由数据库模板预设，其余公开信息由同步程序自动生成。
...
- 分类：不在 Notion 中配置，由 Git 和构建层处理；缺失时统一归入 `notes`
...
3. 创建上面的“新文章”模板，并设为数据库默认模板
4. 将数据库共享给 Notion Integration
5. 将数据库 ID 填入 `NOTION_DATABASE_ID`
